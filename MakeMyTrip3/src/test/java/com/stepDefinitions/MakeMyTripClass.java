...
import com.elementRepository.AgreementPage;
...
//		Assert.assertEquals(driver.getTitle(), title);
	...
	public void user_is_able_to_choose_indian_rupee_option() throws Exception {
		...
		actions.moveToElement(currency).click(currency).click();
	...
	@When("user is click on the Price\\(Lowest First) option")
...
//		webDriverUtility.scrollToPosition(driver, 0, 500);
		bookingPage = propertyPage.goToBookingPage(driver);
//		fileUtility.thread();
//		WebElement propertyTitle = bookingPage.getPropertyTitle();
//		Assert.assertTrue(propertyTitle.isDisplayed(), "Property details page should be displayed");
	...
	public void user_is_able_to_navigate_to_the_next_page() {
		...
		Assert.assertTrue(driver.getPageSource().contains(expectedTitle),
				"User should be navigated to the next page URL");
		Reporter.log("user is navigated to the next page", true);
	}

	@When("User Should be choose the Property")
	public void user_should_be_choose_the_property() throws Exception {
//		webDriverUtility.scrollToPosition(driver, 0, 500);
//		Thread.sleep(5000);
//		bookingPage.shadowElement();
		bookingPage.clickElement(bookingPage.getShareWithFriendsPopUp());
		bookingPage.clickElement(bookingPage.getChoosingProperty());
//		fileUtility.thread();
		Reporter.log("User is navigated to the property Page",true);
		String title=fileUtility.getDataFromExcel("Sheet6", 5, 0);
		webDriverUtility.switchToTabOnTitle(driver, title);
		
	}
	@Then("User is able to click on the book this now button")
	public void user_is_able_to_click_on_the_book_this_now_button() {
		agreementPage=bookingPage.goToAgreementPage(driver);
		webDriverUtility.scrollToPosition(driver, 0, 900);
	}

	@Then("click on user agreement link")
	public void click_on_user_agreement_link() {
		agreementPage.clickElement(agreementPage.getAgreement());
		
		agreementPage.clickElement(agreementPage.feesAndPayment(driver));
	}
...
////		webDriverUtility.scrollToPosition(driver, 0, 1000);
//		fileUtility.thread();
...
//
//	@Then("User is able to click on the book this now button")
//	public void user_is_able_to_click_on_the_book_this_now_button() throws Exception {
//		Reporter.log("User clicked the Book Now Button", true);
//		propertyPage.clickElement(propertyPage.getBookThisNow());
//		String title = fileUtility.getDataFromExcel("Sheet6", 5, 0);
//		Assert.assertEquals(driver.getTitle(), title);
...
//
//	@Then("click on user agreement link")
//	public void click_on_user_agreement_link() throws Exception {
//		webDriverUtility.scrollToPosition(driver, 0, 1000);
//		Reporter.log("User is able to scroll to the Agreement link", true);
//		fileUtility.thread();
//
//		Reporter.log("User clicked the Agreement link", true);
//		agreementPage=propertyPage.goToAgreementPagePage(driver);
//		
//		fileUtility.thread();
//
//		String title = fileUtility.getDataFromExcel("Sheet6", 6, 0);
//		webDriverUtility.switchToTabOnTitle(driver, title);
//
//		Assert.assertEquals(driver.getTitle(), title);
